...
export const timeApi = {
  getRunning: async (): Promise<TimeLog | null> => (await api.get('/time/running')).data.data,
  ...
  logManual: async (taskId: string, data: { startedAt: string; endedAt: string; note?: string }): Promise<TimeLog> =>
    (await api.post('/tasks/' + taskId + '/time', data)).data.data,
  listByTask: async (taskId: string): Promise<TimeLog[]> => (await api.get('/tasks/' + taskId + '/time')).data.data,
